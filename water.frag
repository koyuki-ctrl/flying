...
void main() {
    ...
    // Micro-vagues pour la texture animée
    ...
    // Lignes de crête blanches (haut des vagues)
    float crest = smoothstep(0.35, 0.42, h);
    color = mix(color, vec3(0.8, 0.95, 1.0), crest * 0.7);

    // Reflets stylisés en "bandes" (style cartoon)
    ...
    // Bord des bandes légèrement plus foncé (outline subtil)
